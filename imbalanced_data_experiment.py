...
class0 = df[df['is_type_2'] == 0]
class1 = df[df['is_type_2'] == 1].sample(frac=0.3, random_state=random_state)
imbalanced_df = pd.concat([class0, class1]).sample(frac=1.0, random_state=random_state).reset_index(drop=True)
imbalanced_df.to_excel("train_imbalanced.xlsx", index=False)
features = ['V1real', 'V2real']
X = imbalanced_df[features].values.astype(np.float32)
y = imbalanced_df['is_type_2'].values.astype(np.int64)
