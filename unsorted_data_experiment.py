...
unsorted_df = df.sort_values(by='is_type_2', ascending=False).reset_index(drop=True)
unsorted_df.to_excel("train_unsorted.xlsx", index=False)

features = ['V1real', 'V2real']
X = unsorted_df[features].values.astype(np.float32)
y = unsorted_df['is_type_2'].values.astype(np.int64)
...
with torch.no_grad():
    ...
    print(f"F1-score with unsorted (clustered) data: {f1:.4f}")
    print(f"Accuracy with unsorted (clustered) data: {acc:.4f}")